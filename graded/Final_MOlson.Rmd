...
* My intention was for the function names to be `print.league`, etc
    * When you create something like `l <- league(data, 10, 200, pos, points)`, you can call `plot(l)` and R knows that you really mean `plot.league(l)` -- looks like this was confusing, but you did this correctly
* Task 2.2, points should be calculated with `league$points`; -5
* Task 2, league object doesn't fail gracefully; -4
    * code should fail on things like not enough players
* Task 3, fails to run; -10
* Task 3.1, rather than add helper functions, utilize buildValues by manipulating `league$stats`
* Task 3.3, unnecessary to create both `league.conf.interval` and `conf.interval`
* Task 3.4, `league.conf.interval` is created with `probs`, which isn't required to be length 3; -1
